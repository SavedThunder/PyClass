...
"""A Project By Kyle O'Connor and Bryan Huddleston"""
import random
import sys
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        if (self.place.ant is None) or (self.place.ant.blocked_path == False):
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while len(currentplace.bees) < 1:
            currentplace = currentplace.entrance
            counter += 1
        if (currentplace != hive) and (counter < self.r_max) and (counter > self.r_min):
...
class Water(Place):
    ...
    def add_insect(self,Insect):
        ...
        armorLevel = Insect.armor
        if Insect.watersafe == False:
            Insect.reduce_armor(armorLevel) 
        # END Problem 3A
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor-amount <= 0:
            for b in self.place.bees:
                b.armor -=self.damage
            newlist=[] + self.place.bees
            for b in newlist:
                if b.armor <= 0:
                    self.place.bees.remove(b)
